...
STATE_FILE = os.path.join(CACHE_DIR, "state_groq_v1.json")
...
class State:
    ...
    def get_recent_titles(self): return self.data["recent_titles"]
...
async def generate_image(title, session):
    try:
        ...
        objects = ["hacker silhouette", "broken screen", "warning hologram"]
        ...
        for entry in feed.entries[:2]:
            ...
            try:
                transcript = await asyncio.to_thread(
                    lambda: YouTubeTranscriptApi.list_transcripts(vid).find_transcript(['ru', 'en']).fetch()
                )
...
async def main():
    logger.info("🚀 Starting (Groq FREE mode)...")
